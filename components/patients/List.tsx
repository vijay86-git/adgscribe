...
import { Patient, Pagination, SearchRequestBody } from '@/components/patients/Types'
...
export default function Patients() {
    ...
    const fetchPatients = async (page = 1, debouncedQuery = '') => {
        ...
        const response = await getPatients(requestBody);
        if (response.success) {
            setLoading(false);
            setInitial(false);
            setPatients(response.res.patients.data);
            setPagination(response.res.patients);
        } else {
        ...
        <div className={`${loading && (!initial) ? 'opacity-20' : ''}`}>
            ...
            <div className="overflow-hidden rounded-sm border border-gray-200 border-1">
                <Table className="w-full">
                    ...
                    <TableBody>
                        <PatientRow patients={patients} pagination={pagination} initial={initial} />
                    </TableBody>
                </Table>
            </div>
            <div className="mt-5">
